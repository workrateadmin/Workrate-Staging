# Consuming Memphis Bold in Expo apps

Read `artifacts/memphis-bold/docs/AGENTS.md` first. React Native does
...
`artifacts/memphis-bold/docs/migrating-expo.md` before writing UI.
...
```tsx
import { tokens } from "@workspace/memphis-bold/tokens";
...
```tsx
import { nativeTheme } from "@workspace/memphis-bold/lib/native-theme";
...
import { Button } from "@workspace/memphis-bold/components/native/button";
import { Card } from "@workspace/memphis-bold/components/native/card";
```
...
Import and render
`@workspace/memphis-bold/components/native/button`, then run Expo
