# Migrating Expo UI to Memphis Bold

Read `artifacts/memphis-bold/docs/AGENTS.md` and
`artifacts/memphis-bold/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/memphis-bold/lib/native-theme` and
  `@workspace/memphis-bold/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/memphis-bold/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/memphis-bold` and retains only
