...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  // Map Replit secrets into EXPO_PUBLIC_ vars so they are baked into the
  // production bundle by Metro at build time.
  const clerkKey =
  ...
  if (clerkKey) {
    ...
    console.warn(
      'WARNING: CLERK_PUBLISHABLE_KEY not found — Clerk auth will not work in the production bundle.',
    );
  }

  // API base URL: prefer explicit override, otherwise derive from deployment domain.
  const apiBaseUrl =
    process.env.EXPO_PUBLIC_API_BASE_URL ||
    (process.env.REPLIT_INTERNAL_APP_DOMAIN
      ? `https://${stripProtocol(process.env.REPLIT_INTERNAL_APP_DOMAIN)}`
