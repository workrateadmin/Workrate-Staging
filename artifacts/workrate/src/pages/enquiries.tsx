      ...
      <Link href={`/enquiries/${enq.id}`}>
        <Card className="border-border/60 shadow-sm hover:shadow-md transition-all cursor-pointer bg-card rounded-xl overflow-hidden group-hover:border-primary/30">
          ...
          <CardContent className="p-4 space-y-3">
            {/* Channel / TEST badges */}
            {((enq as any).channel === "whatsapp" || enq.isTest) && (
              <div className="flex items-center gap-1.5 mb-1">
                {(enq as any).channel === "whatsapp" && (
                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-black tracking-widest uppercase rounded bg-[#25D366]/10 text-[#128C7E] border border-[#25D366]/25">
                ...
                {enq.isTest && (
                  <span className="inline-flex w-fit items-center px-1.5 py-0.5 text-[9px] font-black tracking-widest uppercase rounded bg-orange-100 text-orange-700 border border-orange-200">
                    TEST
                  </span>
                )}
              </div>
