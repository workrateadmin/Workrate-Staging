  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="mt-8 rounded-2xl border border-dashed border-border bg-secondary/30 px-8 py-8 flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
        <div className="w-14 h-14 bg-background border border-border rounded-2xl flex items-center justify-center shrink-0 shadow-sm text-2xl">
          🔌
        </div>
...
function IntegrationCard({
  ...
  const connectMutation = useConnectIntegration({
    mutation: {
      onSuccess: () => {
        toast({ title: `${item.name} connected` });
        onRefresh();
      },
      onError: (err: any) => {
        toast({
          title: "Coming soon",
          description: err?.response?.data?.message ?? `${item.name} integration is not yet available.`,
        });
      },
    },
  });
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        >
          {item.provider === "facebook_messenger" && (
      ...
      <div className="flex items-center gap-2 pt-1">
        {isConnected ? (
          <>
            <Button variant="outline" size="sm" className="flex-1 font-semibold rounded-xl h-9 text-xs">
              Configure
            </Button>
            <Button
              variant="ghost"
              size="sm"
              className="font-semibold rounded-xl h-9 text-xs text-muted-foreground hover:text-destructive"
              onClick={() => disconnectMutation.mutate({ provider: item.provider })}
              disabled={isPending}
            >
