...
type Stage = "closed" | "welcome" | "trade" | "chat";

// ── Trade options ─────────────────────────────────────────────────────────────
const TRADES = [
  { id: "Joinery",              label: "Joinery",           icon: "🪵", desc: "Wardrobes, alcoves, bespoke furniture" },
  ...
  { id: "Kitchen Installation", label: "Kitchen",           icon: "🍳", desc: "Supply & fit kitchens, worktops" },
...
export default function WidgetPage() {
  // Detect if this is inside an iframe (for demo mode)
...
function Widget() {
  ...
  // Mark unread when closed and a new AI message arrives
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col items-end gap-3">
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            {/* WorkRate mark */}
            <div className="w-9 h-9 bg-[#2563EB] rounded-xl flex items-center justify-center shadow-lg shrink-0">
        ...
        {stage === "welcome" && (
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div
              ...
              style={{ background: "linear-gradient(160deg, #2563EB 0%, #1D4ED8 60%, #1E293B 100%)" }}
            >
            ...
            <div className="flex-1 bg-white px-5 py-6 flex flex-col gap-4">
              <button
                ...
                className="w-full flex items-center justify-between bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold rounded-xl px-5 py-4 transition-all shadow-md hover:shadow-lg group"
              >
                <div className="flex items-center gap-3">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1.5">
              {TRADES.map((trade) => (
                <button
                  ...
                  disabled={isStarting}
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-100 bg-white hover:border-[#2563EB]/30 hover:bg-blue-50/50 transition-all text-left group disabled:opacity-60"
                >
                  <span className="text-xl shrink-0">{trade.icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-slate-800 group-hover:text-[#2563EB] transition-colors">{trade.label}</p>
                  ...
                  {isStarting && tradeType === trade.id
                    ...
                    : <span className="text-slate-300 group-hover:text-[#2563EB] transition-colors text-sm">→</span>
                  }
                </button>
              ))}
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-slate-50/60">
              {messages.map((msg, i) => (
                <div key={i} className={cn("flex gap-2", msg.role === "customer" ? "justify-end" : "justify-start")}>
                  {msg.role === "assistant" && (
                    <div className="w-7 h-7 bg-[#2563EB] rounded-full flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
              ...
              <div className="bg-white border-t border-slate-100 px-3 py-3 shrink-0">
                ...
                <form onSubmit={sendMessage} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    disabled={isStreaming || isUploadingPhoto}
                    className="flex-1 h-10 px-4 rounded-full bg-slate-100 border-none outline-none text-sm text-slate-800 placeholder:text-slate-400 focus:bg-white focus:ring-2 focus:ring-[#2563EB]/30 transition-all"
                  />
                  <button
                    type="submit"
                    disabled={(isStreaming || isUploadingPhoto) || (!pendingPhoto && !inputValue.trim())}
                    className="w-9 h-9 rounded-full bg-[#2563EB] hover:bg-[#1D4ED8] disabled:bg-slate-200 flex items-center justify-center transition-all shadow-sm shrink-0"
                  >
                    {isUploadingPhoto ? (
      ...
      <button
        ...
        style={{ boxShadow: isOpen ? "0 8px 30px rgba(0,0,0,0.3)" : "0 8px 30px rgba(37,99,235,0.45)" }}
      >
        {/* Unread dot */}
        {unread && !isOpen && (
        ...
        {/* Icon morphs between open/close */}
